...
class StudentTestResultServiceTest {
  ...
  @Mock private StudentTestResultRepository testRepository;
  @Mock private StudentRepository studentRepository;
  @Mock private StudentTestResultMapper testMapper;
  @InjectMocks private StudentTestResultImpl studentTestResultService;
  ...
  void getTestResult() {
    ...
    var student =
        new StudentEntity(studentId, "student", "student@mail", List.of(studentProjectWorkshop));

    when(studentRepository.findById(studentId)).thenReturn(Optional.of(student));

    when(testMapper.testEntityToTestDto(studentTestResultEntity)).thenReturn(studentTestResultDto);
  ...
  void getTestResultWithStudentNotFoundException() {
    ...
    // Act & Assert
    assertThrows(
        StudentNotFoundException.class, () -> studentTestResultService.getTestResult(studentId));
  }
  ...
  void getTestResultWithStudentTestResultNotFoundException() {
    ...
    // Act & Assert
    assertThrows(
        StudentTestResultNotFoundException.class,
  ...
  void createTestResult() {
    ...
    when(studentRepository.findById(studentId)).thenReturn(Optional.of(student));
    when(testRepository.findByStudentProjectWorkshop(studentProjectWorkshop))
  ...
  void createTestResultWithStudentTestResultAlreadyExistException() {
    ...
    when(studentRepository.findById(studentId)).thenReturn(Optional.of(student));
    when(testRepository.findByStudentProjectWorkshop(studentProjectWorkshop))
        .thenReturn(Optional.of(studentTestResultEntity));

    // Act & Assert
    assertThrows(
        StudentTestResultAlreadyExistException.class,
        () -> studentTestResultService.createTestResult(studentTestResultDto, studentId));
  }
}
